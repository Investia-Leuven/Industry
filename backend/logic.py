...
logger = logging.getLogger("investia")
        ...
        try:
            ...
            if is_rate_limit:
                if attempt < max_retries - 1:
                    ...
                    logger.warning(f"{name} rate limited. Retrying in {delay:.1f}s... (Attempt {attempt+1}/{max_retries})")
                    time.sleep(delay)
                    continue
            raise e
...
def get_live_exchange_rates(currencies_tuple):
    """
    Fetches live exchange rates. Note: currencies must be a tuple for lru_cache.
    """
    currencies = list(currencies_tuple)
    """
...
def get_industries_for_sector(sector_key):
    ...
    try:
        ...
        df = fetch_with_retry(fetch_sector_industries, name=f"Sector '{sector_key}'", max_retries=5)
        if df is None or df.empty:
            return {}
        return dict(zip(df["name"], df.index))
    except Exception as e:
        logger.error(f"Failed to get industries for sector {sector_key}: {e}")
        return {}
...
def combine_industry_dataframes(industry_names, industry_keys, data_method):
    ...
    enriched_df, warnings = fetch_additional_company_data(full_df)
...
def fetch_additional_company_data(df_with_symbols):
    ...
    Returns (DataFrame, list of user-facing warning strings).
    """
    ...
    exchange_rates = get_live_exchange_rates(tuple(sorted(used_currencies)))
    
    # Identify failed currencies and warn
    failed_currencies = [cur for cur, rate in exchange_rates.items() if rate is None and cur != "USD"]
    warnings_list = []
    if failed_currencies:
        msg = (
            f"Could not fetch exchange rates for: {', '.join(failed_currencies)}. "
    ...
    return df.round(2), warnings_list
...
def apply_final_sorting_and_formatting(df):
    ...
    # Final cleanup for JSON compatibility (replace NaN/Inf with None)
...
def process_uploaded_tickers(uploaded_file, existing_df):
    ...
    uploaded_data_df, upload_warnings = fetch_additional_company_data(df_symbols)
    ...
    return combined_df, None, upload_warnings
